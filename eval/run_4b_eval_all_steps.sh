#!/usr/bin/env bash
# =============================================================================
# run_4b_eval_all_steps.sh
# Evaluate 4b SFT model across ALL 39 checkpoints on Im2GPS3K.
#
# 8 GPUs → 5 rounds of 8 parallel jobs (8+8+8+8+7).
# Each job: launch SGLang on one GPU, run eval, kill SGLang.
#
# Model:  mp16pro-geo-qwen3-4b-v2-fsdp-fsdp2-sp1-n8-lr2e-5-bs512
# Steps:  200 400 ... 7800 7842
#
# USAGE:
#   bash run_4b_eval_all_steps.sh
#   BASE_GPU=0 BASE_PORT=31000 LABEL_FORMAT=l2 bash run_4b_eval_all_steps.sh
# =============================================================================

set -euo pipefail

EVAL_SCRIPT="/mnt/sh/mmvision/home/jonahli/projects/tusou/eval/eval_sft_granularity.py"
SFT_DIR="/mnt/sh/mmvision/home/jonahli/save/tusou/sft/mp16pro-geo-qwen3-4b-v2-fsdp-fsdp2-sp1-n8-lr2e-5-bs512"
LOG_DIR="/mnt/sh/mmvision/home/jonahli/save/tusou/eval/4b_all_steps/logs"
EVAL_DIR="/mnt/sh/mmvision/home/jonahli/save/tusou/eval/4b_all_steps"
...
launch_one() {
    ...
    local tag="sft_4b_${LABEL_FORMAT}_step${step}"
    ...
    # Skip if valid summary already exists (non-zero parsed)
    ...
    # prefix triggers "!!!!" for well-trained checkpoints due to token-sequence mismatch.
...
# ---------------------------------------------------------------------------
echo "======================================================================"
echo "  4B SFT All-Steps Eval  |  label_format=${LABEL_FORMAT}"
...
for (( batch_start=0; batch_start<TOTAL; batch_start+=N_GPU )); do
    ...
    echo "--- Round $round (jobs $((batch_start+1))–$((batch_end+1)) / $TOTAL) ---"
...
# ---------------------------------------------------------------------------
echo "======================================================================"
echo "  Im2GPS3K — 4B SFT All Steps  (label_format=${LABEL_FORMAT})"
echo "======================================================================"
printf "  %-44s %6s %7s %8s %8s %9s\n" "Tag" "@1km" "@25km" "@200km" "@750km" "@2500km"
printf "  %-44s %6s %7s %8s %8s %9s\n" "---" "----" "-----" "------" "------" "-------"

for step in "${STEPS[@]}"; do
    tag="sft_4b_${LABEL_FORMAT}_step${step}"
