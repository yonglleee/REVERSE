...
def _get_extra_headers() -> dict[str, str]:
    ...
    raw_headers = os.getenv("SEARCH_API_HEADERS")
    if raw_headers:
        try:
            parsed = json.loads(raw_headers)
            if isinstance(parsed, dict):
                extra_headers.update({str(k): str(v) for k, v in parsed.items()})
            else:
                logger.warning("SEARCH_API_HEADERS is not a JSON object; ignoring.")
        except json.JSONDecodeError:
            logger.warning("SEARCH_API_HEADERS is not valid JSON; ignoring.")

    api_key = os.getenv("SEARCH_API_KEY")
    if api_key:
        header_name = os.getenv("SEARCH_API_HEADER", "Authorization")
        prefix = os.getenv("SEARCH_API_PREFIX", "Bearer ")
        extra_headers.setdefault(header_name, f"{prefix}{api_key}")
