...
def _truncate_for_log(text: str, max_len: int = 600) -> str:
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        for match in matches:
            try:
                ...
                logger.error(
                    "Failed to decode tool call: %s",
                    json.dumps(
                        {
                            "reason": "tool_call_json_decode_error",
                            "parser": "hermes",
                            "error_type": type(e).__name__,
                            "error": str(e),
                            "tool_call_output": _truncate_for_log(match),
                        },
                        ensure_ascii=False,
                    ),
                )
                    ...
                    json.dumps(
                        {
                            ...
                            "error_type": type(e).__name__,
                            "error": str(e),
                            "tool_name_output": _truncate_for_log(str(match[0]) if len(match) > 0 else ""),
                            "tool_args_output": _truncate_for_log(str(match[1]) if len(match) > 1 else ""),
                        },
                        ensure_ascii=False,
                    ),
                )
