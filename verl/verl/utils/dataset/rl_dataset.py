...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # Optional root dir for relative image paths (DATA_ROOT env var as fallback)
        self.image_root = config.get("image_root", os.environ.get("DATA_ROOT", ""))
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        if image_key in doc and doc[image_key]:
                            def _resolve_image(img, image_root):
                                if image_root and isinstance(img, dict):
                                    for k in ("image", "image_url"):
                                        if k in img:
                                            p = img[k]
                                            if isinstance(p, str) and not os.path.isabs(p):
                                                img = dict(img)
                                                img[k] = os.path.join(image_root, p)
                                            break
                                return img
                            images = [
                                process_image(_resolve_image(image, self.image_root),
    ...
    def __getitem__(self, item):
        ...
        # Filter out None-valued entries that appear after parquet serialization of nozoom samples
        # (Dataset.from_list pads missing keys with None when schema is inferred from zoom samples)
        if isinstance(tools_kwargs, dict):
            tools_kwargs = {k: v for k, v in tools_kwargs.items() if v is not None}
    ...
    async def process_vision_info(
        ...
        # Resolve relative image paths using DATA_ROOT (injected via ray runtime_env or env var)
        data_root = os.environ.get("DATA_ROOT", config.get("image_root", "") if config else "")
        if data_root:
            resolved = []
            for msg in messages:
                msg = dict(msg)
                    ...
                    for item in msg["content"]:
                        if isinstance(item, dict):
                            ...
                            for k in ("image", "image_url"):
                                if (
                                    k in item
                                    and isinstance(item[k], str)
                                    and item[k]
                                    and not item[k].startswith(("http", "/", "data:", "file:"))
                                ):
                                    item[k] = os.path.join(data_root, item[k])
