...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(
        ...
        # Optional root directory for relative image paths (DATA_ROOT env var as fallback)
        import os as _os
        self.image_root = config.get("image_root", _os.environ.get("DATA_ROOT", ""))
    ...
    def _build_messages(self, example: dict):
        ...
        for message in messages:
            ...
            for segment in segments:
                if segment == "<image>":
                    img_entry = images[image_offset]
                    # Prepend image_root to relative paths
                    if self.image_root and isinstance(img_entry, dict):
                        import os as _os
                        for key in ("image", "image_url"):
                            if key in img_entry:
                                p = img_entry[key]
                                if isinstance(p, str) and not _os.path.isabs(p):
                                    img_entry = dict(img_entry)
                                    img_entry[key] = _os.path.join(self.image_root, p)
                                break
                    image = process_image(img_entry, image_patch_size=self.image_patch_size)
