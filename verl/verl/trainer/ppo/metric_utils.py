...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Per-tool reward and pos/neg statistics from tool_reward_metas
    if "tool_reward_metas" in batch.non_tensor_batch:
        tool_reward_metas = batch.non_tensor_batch["tool_reward_metas"]  # array of lists of dicts
        from collections import defaultdict
        tool_stats = defaultdict(lambda: {
            "reward": [],
            "crop_iou": [],   # IOU between model's crop bbox and gt bbox (image_search only)
            "cache_hit": [], "f1_score": [], "call_count": 0
        })
        for sample_metas in tool_reward_metas:
            ...
            for meta in sample_metas:
                ...
                # iou = crop quality: how well model's bbox overlaps with gt bbox
        ...
        for tool_name, stats in tool_stats.items():
            ...
            metrics[f"{prefix}/call_count"] = stats["call_count"]
            if stats["reward"]:
                metrics[f"{prefix}/reward/mean"] = sum(stats["reward"]) / len(stats["reward"])
            if stats["crop_iou"]:
                metrics[f"{prefix}/crop_iou/mean"] = sum(stats["crop_iou"]) / len(stats["crop_iou"])
                metrics[f"{prefix}/crop_iou/hit_rate"] = sum(1 for v in stats["crop_iou"] if v >= 0.5) / len(stats["crop_iou"])
            if stats["cache_hit"]:
                metrics[f"{prefix}/cache_hit_rate"] = sum(stats["cache_hit"]) / len(stats["cache_hit"])
            if stats["f1_score"]:
                metrics[f"{prefix}/f1_score/mean"] = sum(stats["f1_score"]) / len(stats["f1_score"])

        # Total tool reward per sample
        if "tool_rewards" in batch.non_tensor_batch:
            tool_rewards_per_sample = batch.non_tensor_batch["tool_rewards"]
            all_rewards = []
            for sample_rewards in tool_rewards_per_sample:
            ...
            if all_rewards:
                metrics["tool/total_reward/mean"] = sum(all_rewards) / len(all_rewards)
